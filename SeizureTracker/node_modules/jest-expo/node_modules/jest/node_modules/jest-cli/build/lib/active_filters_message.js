...
const activeFilters = function(globalConfig) {
  ...
  if (testNamePattern || testPathPattern) {
    const filters = [
      testPathPattern
        ? (_chalk || _load_chalk()).default.dim('filename ') +
          (_chalk || _load_chalk()).default.yellow('/' + testPathPattern + '/')
        : null,
      testNamePattern
        ? (_chalk || _load_chalk()).default.dim('test name ') +
          (_chalk || _load_chalk()).default.yellow('/' + testNamePattern + '/')
        : null
    ]
      .filter(f => f)
      .join(', ');
    ...
    const messages = [
      '\n' +
        (_chalk || _load_chalk()).default.bold('Active Filters: ') +
        filters
    ];
>>>>>>> master
