...
const printDisplayName = (exports.printDisplayName = config => {
  ...
  if (displayName) {
    return (_chalk || _load_chalk()).default.supportsColor
      ? (_chalk || _load_chalk()).default.reset.inverse.white(
          ` ${displayName} `
        )
      : displayName;
  }
...
const trimAndFormatPath = (exports.trimAndFormatPath = (
  pad,
  config,
  testPath,
  columns
...
=======
  if (
    (dirname + (_path || _load_path()).default.sep + basename).length <=
    maxLength
  ) {
    return (0, (_slash || _load_slash()).default)(
      (_chalk || _load_chalk()).default.dim(
        dirname + (_path || _load_path()).default.sep
      ) + (_chalk || _load_chalk()).default.bold(basename)
    );
>>>>>>> master
  }
...
const relativePath = (exports.relativePath = (config, testPath) => {
  ...
  // do not have config.cwd, only config.rootDir. Try using config.cwd, fallback
  // to config.rootDir. (Also, some unit just use config.rootDir, which is ok)
  testPath = (_path || _load_path()).default.relative(
...
=======
  ...
  const suites =
    ...
    (suitesFailed
      ? (_chalk || _load_chalk()).default.bold.red(`${suitesFailed} failed`) +
        ', '
      : '') +
    (suitesPending
      ...
      : '') +
    (suitesPassed
      ? (_chalk || _load_chalk()).default.bold.green(`${suitesPassed} passed`) +
        ', '
      : '') +
  ...
  const tests =
    ...
    (testsFailed
      ? (_chalk || _load_chalk()).default.bold.red(`${testsFailed} failed`) +
        ', '
      : '') +
    (testsPending
      ? (_chalk || _load_chalk()).default.bold.yellow(
      ...
      : '') +
    (testsPassed
      ? (_chalk || _load_chalk()).default.bold.green(`${testsPassed} passed`) +
        ', '
      : '') +
    `${testsTotal} total`;
...
const renderTime = (runTime, estimatedTime, width) => {
  ...
  const renderedTime =
    estimatedTime && runTime >= estimatedTime + 1
      ...
      : runTime + 's';
  let time =
    (_chalk || _load_chalk()).default.bold(`Time:`) + `        ${renderedTime}`;
>>>>>>> master
